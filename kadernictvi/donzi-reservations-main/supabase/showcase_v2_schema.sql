...
CREATE TABLE IF NOT EXISTS public.portfolio_poptavky (
  ...
  barbershop_id BIGINT REFERENCES public.showcase_barbershops (id) ON DELETE SET NULL,
  name TEXT NOT NULL,
  email TEXT NOT NULL,
...
COMMENT ON TABLE public.showcase_barbershop_admins IS
  'Propojení auth.users s jedním barbershopem. Po přihlášení admin vidí jen svůj barbershop_id.';
...
-- Přepočet výdělků z showcase_trzby pro daný měsíc
...
BEGIN
  WITH deleted AS (
    DELETE FROM public.showcase_rezervace
...
CREATE POLICY "authenticated_select_portfolio_poptavky"
  ...
  USING (
    ...
    OR barbershop_id = public.showcase_current_barbershop_id()
  );
...
-- =============================================================================
-- 1) INSERT INTO showcase_barbershops (name, slug, email) VALUES ('Salon XY', 'salon-xy', 'info@xy.cz');
-- 2) INSERT INTO showcase_services (barbershop_id, name, price, duration_minutes) VALUES (2, 'Pánský střih', 650, 45);
-- 3) V Supabase Auth vytvoř uživatele → INSERT INTO showcase_barbershop_admins (barbershop_id, user_id, login_label) VALUES (2, 'UUID', 'salon-xy');
