...
COMMENT ON COLUMN public.kadernictvi.sms_unit_cost IS
  'Základní jednotka pro vyúčtování 1 SMS salónu (Kč), např. 1';
COMMENT ON COLUMN public.kadernictvi.sms_billing_multiplier IS
...
CREATE TABLE IF NOT EXISTS public.kadernictvi_sms (
  id BIGSERIAL PRIMARY KEY,
  kadernictvi_id BIGINT NOT NULL REFERENCES public.kadernictvi (id) ON DELETE CASCADE,
  rezervace_id UUID REFERENCES public.kadernictvi_rezervace (id) ON DELETE SET NULL,
  phone TEXT,
...
CREATE INDEX IF NOT EXISTS kadernictvi_sms_barbershop_sent_idx
  ON public.kadernictvi_sms (kadernictvi_id, sent_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS kadernictvi_sms_rezervace_uidx
  ON public.kadernictvi_sms (rezervace_id)
...
UPDATE public.kadernictvi_sms v
SET
...
WHERE b.id = v.kadernictvi_id;

-- Zpětné doplnění chybějících SMS
INSERT INTO public.kadernictvi_sms (
  kadernictvi_id,
  rezervace_id,
  phone,
  unit_cost,
...
JOIN public.kadernictvi b ON b.id = r.kadernictvi_id
WHERE r.sms_sent = TRUE
  AND r.kadernictvi_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.kadernictvi_sms v WHERE v.rezervace_id = r.id
  );

-- Kontrola: SELECT kadernictvi_id, COUNT(*), SUM(amount) FROM kadernictvi_sms GROUP BY 1;
